...
export const ListCompanies = ({ data, setLoading }) => {
  ...
  const onToast = (s) => {
    if ('Delete Company Successfull!!' == s || 'Update Company Successfull!!' == s) {
      toast.success(s, {
        position: "top-center",
        autoClose: 5000,
  ...
  const handleDeleteRow = async (row) => {
    const response = await deleteData({
      "url": `home/company/${row.id}`
    });
    const data = await response.json();
    
    if (response.status == 200) {
      onToast("Delete Company Successfull!!")
  ...
  const handleEditRow = async (row) => {
    ...
    if (response.status == 200) {
      onToast("Update Company Successfull!!")
      window.location.reload();
    } else {
      onToast("Something went wrong!!")
    }
  ...
  const ExpandedComponent = ({ data }) =>
    <div className="list-container">
      <div className="footer-brand">
        ...
        <li className="footer-item logo">
          <span className="span">Location</span> : <span>{data.location}</span>
        </li>
        <li className="footer-item logo">
          <span className="span">Registration Id</span> : <span>{data.registrationId}</span>
        </li>
        <li className="footer-item logo">
          <span className="span">Established Year</span> : <span>{data.establishedYear}</span>
        </li>
        <li className="footer-item logo">
