...
export const ListEmployees = ({ data, setLoading }) => {
  ...
  const onToast = (s) => {
    if ('Delete Employee Successfull!!' == s || 'Update Employee Successfull!!' == s) {
      toast.success(s, {
        position: "top-center",
        autoClose: 5000,
  ...
  const handleDeleteRow = async (row) => {
    const response = await deleteData({
      "url": `employee/delete/${row.id}`
    });
    const data = await response.json();

    if (response.status == 200) {
      onToast("Delete Employee Successfull!!")
  ...
  const handleEditRow = async (row) => {
    ...
    if (response.status == 200) {
      onToast("Update Employee Successfull!!")
      window.location.reload();
    } else {
      onToast("Something went wrong!!")
    }
  ...
  const handleGenerateRow = async (row) => {
    const response = await postData({
      "url": "employee/generate/certificate",
      "data": row,
      "isJson": true
    });
    const data = await response.json();
  ...
  const handleDownloadRow = async (row) => {
    const response = await postData({
      "url": "employee/download/certificate",
      "data": row,
      "isJson": true
    });
  ...
  const ExpandedComponent = ({ data }) =>
    <div className="list-container">
      <div className="footer-brand">
        ...
        <li className="footer-item logo">
          <span className="span">Company Name</span> : <span>{data.company.name}</span>
        </li>
        <li className="footer-item logo">
          <span className="span">Joining Date</span> : <span>{data.joiningDate}</span>
        </li>
        <li className="footer-item logo">
          <span className="span">Department</span> : <span>{data.department}</span>
        </li>
        <li className="footer-item logo">
          <span className="span">Designation</span> : <span>{data.designation}</span>
        </li>
        <li className="footer-item logo">
